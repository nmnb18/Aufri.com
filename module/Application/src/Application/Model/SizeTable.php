...
class SizeTable extends AbstractDbTable {
    ...
    const TABLE_NAME = 'aufri_product_size';
    ...
    public function save(Size $size) {
        ...
        if ($size->getProductSizeId() > 0) {
            $id = $size->getProductSizeId();
            $this->tableGateway->update($data, array('aufri_product_size_id' => $id));
        } else {
        ...
        return $this->getOne(array('aufri_product_size_id' => $id));
    }
}
