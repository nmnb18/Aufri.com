...
class CategoryTable extends AbstractDbTable {
    ...
    const TABLE_NAME = 'aufri_product_category';
    ...
    public function save(Category $category) {
        $data = $category->getRawData();
        if ($category->getProductCategoryId() > 0) {
            $id = $category->getProductCategoryId();
            $this->tableGateway->update($data, array('aufri_product_category_id' => $id));
        } else {
        ...
        return $this->getOne(array('aufri_product_category_id' => $id));
    }
}
